...
#endif
BoschSensorClass::BoschSensorClass() {
#ifdef TARGET_ARDUINO_NANO33BLE
  BMI270_INT1 = p11;
#endif
}

void BoschSensorClass::debug(Stream &stream) { _debug = &stream; }
#ifdef __MBED__
...
void BoschSensorClass::end() {}

bool BoschSensorClass::begin_bmi270(uint8_t i2c_addr, TwoWire *wire) {
  _agwire = wire;
  // _agwire->begin();

  bmi2.chip_id = i2c_addr;
  ...
  accel_gyro_dev_info._wire = _agwire;
  accel_gyro_dev_info.dev_addr = bmi2.chip_id;

  int8_t r1 = bmi270_init(&bmi2);
  print_rslt(r1);

  int r2 = configure_sensor(&bmi2);
  print_rslt(r2);

  return ((r1 == BMI2_OK) && (r2 == BMI2_OK));
}

bool BoschSensorClass::begin_bmm150(uint8_t i2c_addr, TwoWire *wire) {
  _mwire = wire;
  // _mwire->begin();
  bmm1.chip_id = i2c_addr;
  ...
  mag_dev_info._wire = _mwire;
  mag_dev_info.dev_addr = bmm1.chip_id;

  int r1 = bmm150_init(&bmm1);
  print_rslt(r1);

  int r2 = configure_sensor(&bmm1);
  print_rslt(r2);
...
int8_t BoschSensorClass::bmi2_i2c_read(uint8_t reg_addr, uint8_t *reg_data,
...
int8_t BoschSensorClass::bmi2_i2c_write(uint8_t reg_addr,
